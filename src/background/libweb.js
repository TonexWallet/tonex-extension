/* eslint-disable */
function getLibWeb(options) {
    let onResponse = null;

    const setResponseHandler = (handler) => {
        onResponse = handler;
    }

    function core_response_handler(request_id, params_json, response_type, finished) {
        if(!onResponse){
            throw 'Response handler is not initialized';
        }
        onResponse({
    ...
    return () => {
        ...
        const library = {
                ...
                return new Promise((resolve) => {
                    ...
                    if (initComplete) {
                        actions.createContext({
                            result: lib.core_create_context(configJson),
                            requestId: requestId,
                        })
                    }
                });
            ...
            destroyContext: (context) => {
                lib.core_destroy_context(context);
            },
        ...
        const actions = {
            init: () => {
                ...
                for (const [requestId, request] of createContextRequests.entries()) {
                    actions.createContext({
                        result: lib.core_create_context(request.configJson),
                        requestId: requestId,
                    })
                }
            },

            createContext: ({requestId, result}) => {
                const request = createContextRequests.get(requestId);
                if (request) {
                    createContextRequests.delete(requestId);
            ...
            response: ({paramsJson, requestId, responseType, finished}) => {
                if (responseHandler) {
                    if (paramsJson.charCodeAt(0) === 0xFEFF) {
                        paramsJson = paramsJson.substr(1);
                    }
                    responseHandler(requestId, paramsJson, responseType, finished);
                }
            }
        }
        ...
        (async () => {
            ...
            await lib.init(wasmModule);
            actions.init();

            debugLog(`compile time ${Date.now() - e}`);
        })();

        lib.setResponseHandler(actions.response);

        return Promise.resolve(library);
    }
}

export default getLibWeb;
